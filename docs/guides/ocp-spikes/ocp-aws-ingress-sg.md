# Spike to Support Security Groups on NLB Ingress
...
- Create release to file
```sh
# oc adm release new -a $PULL_SECRET_FILE \
# --from-release quay.io/openshift-release-dev/ocp-release:4.19.0-ec.4-x86_64 \
#   aws-cloud-controller-manager=quay.io/mrbraga/cloud-controller-manager-aws:v1.20.0-alpha.0-922-g425c1c5b-dirty \
#   --to-image quay.io/mrbraga/ocp-release:SPLAT-2113-v0
#   --to-file quay.io_mrbraga_ocp-release_SPLAT-2113-v0

podman login -u mtulio docker.io --authfile $PULL_SECRET_FILE
# give a pass

OCP_RELEASE_BASE=quay.io/openshift-release-dev/ocp-release:4.19.0-ec.4-x86_64   

RELEASE_NEW=docker.io/mtulio/ocp-release:SPLAT-2113-v0
RELEASE=quay.io/mrbraga/ocp-release:SPLAT-2113-v0

oc adm release new -a $PULL_SECRET_FILE \
--from-release $OCP_RELEASE_BASE \
aws-cloud-controller-manager=quay.io/mrbraga/cloud-controller-manager-aws:v1.20.0-alpha.0-922-g425c1c5b-dirty   --to-image $RELEASE_NEW


```

- Deploy a image to public registry

```sh
podman pull $RELEASE_NEW
podman tag $RELEASE_NEW $RELEASE
podman push -f $RELEASE
```

- Install clients

```sh
oc adm release extract -a $PULL_SECRET_FILE --tools quay.io/openshift-release-dev/ocp-release:4.19.0-ec.4-x86_64
...
```sh
OPENSHIFT_INSTALL_RELEASE_IMAGE_OVERRIDE="$RELEASE" ./openshift-install create cluster --dir $INSTALL_DIR

# CHANGE ME
version=v7
export OPENSHIFT_INSTALL_RELEASE_IMAGE_OVERRIDE="registry.build07.ci.openshift.org/ci-ln-19b4b6t/release:latest"
...
- Deploy a service with NLB and custom SG

```sh

```

- Check if CCM allows it
...
# TODO v15:
- test create service with managed mode
- test delete and check if SG will be deleted
...
- Ensure AWS account is linked to an RH ID to get ROSA HCP works
...
```sh
ROSA_HCP_CLUSTER_NAME=mrb-rosa1

# Network
# 1.2.1. Creating a Virtual Private Cloud for your ROSA with HCP clusters
rosa create network --param Region=us-east-1 --param Name=$ROSA_HCP_CLUSTER_NAME

INFO[0176] Resource: SecurityGroup, Type: AWS::EC2::SecurityGroup, ID: sg-0d60408a0195a0a87 
INFO[0176] Resource: SubnetPrivate1, Type: AWS::EC2::Subnet, ID: subnet-014d8fe249743a144 
INFO[0176] Resource: SubnetPublic1, Type: AWS::EC2::Subnet, ID: subnet-09ce711942322d51e 
INFO[0176] Resource: VPC, Type: AWS::EC2::VPC, ID: vpc-071a12af3f7bf7e58 
INFO[0176] Stack mrb-rosa1 created 

# 1.2.3. Creating the account-wide STS roles and policies
export ACCOUNT_ROLES_PREFIX=$ROSA_HCP_CLUSTER_NAME
rosa create account-roles --hosted-cp

# 1.2.4. Creating an OpenID Connect configuration
rosa create oidc-config --mode=auto --yes

export OIDC_ID=2ik2hasj3mubc3o1e9dctg6emf490qqm

# 1.2.5. Creating Operator roles and policies
export OPERATOR_ROLES_PREFIX=$ROSA_HCP_CLUSTER_NAME
AWS_ACCOUNT_ID=$(aws sts get-caller-identity --query 'Account' --output text)


rosa create operator-roles --yes \
--hosted-cp \
...
export SUBNET_IDS=subnet-014d8fe249743a144,subnet-09ce711942322d51e 

cat << EOF
ROSA_HCP_CLUSTER_NAME=$ROSA_HCP_CLUSTER_NAME
OPERATOR_ROLES_PREFIX=$OPERATOR_ROLES_PREFIX
OIDC_ID=$OIDC_ID
SUBNET_IDS=$SUBNET_IDS
EOF

rosa create cluster --cluster-name=$ROSA_HCP_CLUSTER_NAME \
    --mode=auto --hosted-cp \
