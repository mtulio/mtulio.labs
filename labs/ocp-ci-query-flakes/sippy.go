...
type SippyTestsRequestFilter struct {
	// {"items":[{"columnField":"name","operatorValue":"equals","value":"test_name"}]}
...
func (a *SippyAPI) QueryTests(r *SippyTestsRequestInput) (*SippyTestsRequestOutput, error) {
	...
	baseUrl, err := url.Parse("https://sippy.dptools.openshift.org/api/tests")
	if err != nil {
	...
	if err != nil {
		log.Fatalf("Couldn't create the request: %+v", err)
	}
	res, err := a.client.Do(req)
	if err != nil {
		log.Fatalf("Couldn't call URL %s: %+v", baseUrl.String(), err)
	}
	...
	if err := json.Unmarshal([]byte(body), &sippyResponse); err != nil {
		log.Fatalf("Couldn't unmarshal response body: %+v \nBody: %s", string(body), err)
		log.Fatal(err)
	}
